...
const { sendContactFormEmail, sendFranchiseOpportunityEmail } = require('../utils/emailService');
...
const submitCreditRepairForm = async (req, res) => {
  try {
    ...
    const { error } = creditRepairSchema.validate(req.body, { abortEarly: false });
    if (error) {
    ...
    const { fullName, email, mobileNumber, city, state, problemType, creditScore, message } = req.body;
    ...
    try {
      ...
      console.error('Failed to sync credit repair data with Google Sheets:', syncError);
...
const submitContactForm = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Failed to sync contact form data with Google Sheets:', syncError);
      // Don't fail the request if Google Sheets sync fails
    }
...
const submitFranchiseOpportunityForm = async (req, res) => {
  try {
    // Validate request body
    const { error } = franchiseOpportunitySchema.validate(req.body, { abortEarly: false });
    if (error) {
      const errorMessages = error.details.map(detail => detail.message);
      return res.status(400).json({
        message: 'Validation error',
    ...
    const { fullName, mobileNumber, email, city, state, profession, message, consent } = req.body;
    ...
    try {
      ...
      console.error('Failed to send franchise opportunity email to admin:', emailError);
      // Don't fail the request if email sending fails
    }
    ...
    try {
      ...
      console.error('Failed to sync franchise opportunity data with Google Sheets:', syncError);
      // Don't fail the request if Google Sheets sync fails
    }
...
const submitBusinessForm = async (req, res) => {
  try {
    // Validate request body
    const { error } = businessFormSchema.validate(req.body, { abortEarly: false });
    if (error) {
      const errorMessages = error.details.map(detail => detail.message);
      return res.status(400).json({
        message: 'Validation error',
    ...
    const { 
      ...
      city, state, pincode, occupation, monthlyIncome, fullAddress,
